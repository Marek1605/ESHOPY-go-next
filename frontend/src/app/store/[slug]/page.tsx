...
import { ShoppingCart, Search, Heart, X, Star, Sparkles, Package, Truck, Shield, Phone, Mail, MapPin, Minus, Plus, Trash2 } from 'lucide-react';
import { useCart } from '@/lib/store';

const SHOPS: Record<string, { name: string; color: string; products: any[] }> = {
  'demo': {
    name: 'Demo Shop',
    color: '#3b82f6',
    products: [
...
export default function StorePage() {
  ...
  return (
    <div className="min-h-screen bg-white text-gray-900">
      {/* Cart Overlay */}
      {isOpen && <div className="fixed inset-0 bg-black/50 z-50" onClick={() => setCartOpen(false)} />}
      ...
      <div className={`fixed top-0 right-0 bottom-0 w-full max-w-md bg-white z-50 transform transition-transform ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="flex flex-col h-full">
          <div className="flex items-center justify-between p-4 border-b">
            <h2 className="text-lg font-bold">Košík ({items.length})</h2>
            <button onClick={() => setCartOpen(false)} className="p-2 hover:bg-gray-100 rounded-lg"><X className="w-5 h-5" /></button>
          </div>
                  ...
                  <div key={item.id} className="flex gap-4 p-3 bg-gray-50 rounded-lg">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-2 mt-2">
                        <button onClick={() => updateQuantity(item.id, item.quantity - 1)} className="p-1 hover:bg-gray-200 rounded"><Minus className="w-4 h-4" /></button>
                        <span className="w-8 text-center text-sm">{item.quantity}</span>
                        <button onClick={() => updateQuantity(item.id, item.quantity + 1)} className="p-1 hover:bg-gray-200 rounded"><Plus className="w-4 h-4" /></button>
          ...
          {items.length > 0 && (
            <div className="border-t p-4 space-y-4">
              <div className="flex justify-between text-lg font-bold"><span>Celkom:</span><span>€{cartTotal.toFixed(2)}</span></div>
      ...
      <header className="bg-white border-b sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
          <Link href={`/store/${slug}`} className="flex items-center gap-2">
            <div className="w-10 h-10 rounded-lg flex items-center justify-center" style={{ background: primaryColor }}><Sparkles className="w-6 h-6 text-white" /></div>
            <span className="font-bold text-xl">{shop.name}</span>
          </Link>
          <div className="flex items-center gap-4">
            <div className="hidden md:block relative w-64">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400" />
              <input type="text" placeholder="Hľadať..." className="w-full pl-10 pr-4 py-2 rounded-lg bg-gray-100 text-sm focus:outline-none" />
            </div>
            <button className="p-2 hover:bg-gray-100 rounded-lg"><Heart className="w-6 h-6" /></button>
            <button onClick={() => setCartOpen(true)} className="p-2 hover:bg-gray-100 rounded-lg relative">
      ...
      {/* Hero */}
      <section className="py-16 text-white text-center" style={{ background: `linear-gradient(to right, ${primaryColor}, ${primaryColor}dd)` }}>
        ...
        <div className="max-w-7xl mx-auto px-4">
          <div className="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {filteredProducts.map(product => (
              <div key={product.id} className="bg-white rounded-xl shadow-lg hover:shadow-xl overflow-hidden transition-all">
                ...
                <div className="p-4">
                  ...
                  <button onClick={() => handleAddToCart(product)} className="w-full mt-3 py-2 text-white font-medium rounded-lg transition hover:opacity-90" style={{ background: primaryColor }}>Do košíka</button>
                </div>
              </div>
            ))}
          </div>
      ...
      <footer className="py-12 bg-gray-900 text-white">
        <div className="max-w-7xl mx-auto px-4 grid md:grid-cols-4 gap-8">
          ...
          <div>
            ...
            <div className="space-y-2 text-gray-400"><p>O nás</p><p>Obchodné podmienky</p><p>Ochrana súkromia</p></div>
          </div>
          <div>
            <h4 className="font-semibold mb-4">Newsletter</h4>
            <p className="text-gray-400 mb-4">Prihláste sa a získajte 10% zľavu</p>
            <div className="flex">
              <input type="email" placeholder="Váš email" className="flex-1 px-4 py-2 rounded-l-lg bg-gray-800 text-white" />
