    ...
    <div className="min-h-screen aurora-bg flex items-center justify-center p-4 py-12">
      <div className="w-full max-w-md">
        ...
        <div className="flex items-center justify-center gap-2 mb-8">
          {[1, 2].map((s) => (
            <div key={s} className="flex items-center gap-2">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-medium ${step >= s ? 'bg-gradient-to-r from-blue-500 to-purple-600 text-white' : 'bg-slate-800 text-gray-400'}`}>
              ...
              <>
                <div className="form-group">
                  ...
                  <div className="relative">
                    ...
                    <input type={showPassword ? 'text' : 'password'} value={formData.password} onChange={(e) => setFormData({ ...formData, password: e.target.value })} className="input-field pl-12 pr-12" placeholder="••••••••" required />
        ...
        <p className="text-center mt-8 text-gray-400">
          Už máte účet? <Link href="/login" className="text-blue-400 hover:text-blue-300 font-medium">Prihláste sa</Link>
        </p>
      </div>
    </div>
  );
