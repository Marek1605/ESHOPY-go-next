...
import { Sparkles, LayoutDashboard, Package, ShoppingCart, Users, BarChart3, Settings, Palette, LogOut, Menu, X, Bell, Search, Store, ChevronDown } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-slate-950">
          ...
          <div className="p-4 border-b border-slate-800">
            <button className="w-full flex items-center justify-between p-3 bg-slate-800 rounded-xl hover:bg-slate-700">
              <div className="flex items-center gap-3">
                ...
                <span className="text-sm font-medium truncate">Môj obchod</span>
              </div>
        ...
        <nav className="p-4 space-y-1">
          {menuItems.map((item) => (
            <Link key={item.href} href={item.href} className={`sidebar-item ${pathname === item.href ? 'active' : ''}`}>
              ...
              {sidebarOpen && <><span className="flex-1">{item.label}</span>{item.badge && <span className="px-2 py-0.5 bg-blue-500 text-white text-xs rounded-full">{item.badge}</span>}</>}
            </Link>
          ))}
        </nav>
      ...
      <header className="lg:hidden fixed top-0 left-0 right-0 h-16 bg-slate-900 border-b border-slate-800 z-50 flex items-center justify-between px-4">
        ...
        <button className="p-2 hover:bg-slate-800 rounded-lg relative"><Bell className="w-6 h-6" /><span className="absolute top-1 right-1 w-2 h-2 bg-red-500 rounded-full" /></button>
      </header>
      {mobileOpen && (
      ...
      <main className={`min-h-screen pt-16 lg:pt-0 ${sidebarOpen ? 'lg:ml-64' : 'lg:ml-20'}`}>
        <div className="hidden lg:flex h-16 items-center justify-between px-6 border-b border-slate-800 bg-slate-900/50">
          <div className="relative w-96">
            ...
            <input type="text" placeholder="Hľadať produkty, objednávky..." className="input-field pl-10" />
          </div>
          <div className="flex items-center gap-4">
            <button className="p-2 hover:bg-slate-800 rounded-lg relative"><Bell className="w-5 h-5 text-gray-400" /><span className="absolute top-1 right-1 w-2 h-2 bg-red-500 rounded-full" /></button>
            <Link href="/store/preview" target="_blank" className="btn-secondary text-sm py-2"><Store className="w-4 h-4" />Zobraziť obchod</Link>
          </div>
        </div>
